...
桌面端的主界面以 Coding Agent 对话为中心。用户提交的是最终目标，例如“修复订单查询缺少租户过滤”；RepoPilot 在同一条会话中持续反馈处理进度，并在结束时只给出用户真正需要的结论：任务是否完成、是否产生真实代码修改、Maven 验证是否通过，以及无法继续时的明确原因。
...
- 对话草稿只保存显示标题、项目归属、模式和归档状态，不保存模型上下文、代码正文或密钥。真实 Agent 任务仍必须绑定项目、权限快照和受控工作区。
