...
当前候选版本：`0.1.17 / Pre-Alpha`。第一版 MVP 的后端闭环已经跑通；Windows x64 release 与 NSIS 安装包已成功构建，并完成隔离目录中的安装、启动、sidecar 退出和卸载烟雾测试。桌面端采用 Windows GUI 子系统和无控制台的 Python sidecar，已安装应用启动时不会额外弹出命令行窗口。当前版本支持普通多轮对话、只读代码分析自动总结和受控代码修改任务；当前代码版本的 15 项端到端评测已一次性执行并全部符合预期，升级迁移等交付验收仍在进行，尚不建议用于生产仓库。
...
桌面端的主界面以 Coding Agent 对话为中心。用户提交的是最终目标，例如“修复订单查询缺少租户过滤”；RepoPilot 在同一条会话中持续反馈处理进度，并在任务结束时把受控终态总结直接写入聊天流。终态后输入框重新开放，用户可继续提出下一项代码任务；下一轮会继承此前用户任务和助手总结，但同一对话同一时刻只允许一个任务运行，避免工作区和审批状态相互覆盖。

- `PASSED`：存在真实 Diff，且已保存成功的 Maven 验证证据，才会显示“目标已完成”；
- `UNVERIFIED`：只完成研究或计划，不能当作 Bug 已修复；
- `FAILED` / `BLOCKED`：明确说明任务未完成及阻断原因；
- 计划、候选文件、执行轨迹、Diff、验证输出和 Evidence 默认收起，用户可在“查看详情”或会话内按需展开；
...
桌面端默认处于“对话”模式：普通消息不会读取仓库、创建 Agent 任务、申请审批或执行命令。用户手动切换到“分析代码”后，RepoPilot 才会调用受控只读工具，并在同一会话中直接输出带证据的 `UNVERIFIED` 总结；只有“修改代码”会进入计划审批、执行审批、结构化补丁和 Maven 验证。用户消息显示在右侧，助手回复显示在左侧；任务细节、Diff 和审计证据按需展开。
