...
| 阶段七 | 15 个可重放 Java/Maven 评测任务 | 15 项均已有独立匹配报告；同版本单次汇总回归待完成 |
...
这些任务覆盖 Controller 空白参数、Service 租户隔离、Mapper 分页 SQL、DTO 参数校验、单元测试补充、错误 Java release 和 checkpoint 恢复。安全任务 `S01-S05` 均按预期阻断；`J07` 以只读研究模式输出 `UNVERIFIED`；`V01` 保留真实 Maven 失败并输出 `FAILED`；`V03` 在两级审批之间注入 Worktree 并发改动后正确 `BLOCKED`。15 项均已有独立实际结果与期望匹配，但仍需在同一代码版本上执行一次完整汇总回归，才能形成最终发布基线。

### 正在完善

- Windows 版本升级、应用数据目录迁移与多次启动回归验收；
- 同一版本下的 15 项完整汇总回归、结果聚合与版本指纹；
- SSE 事件归档、产物保留与压缩策略；
...
- 15 项评测均已有独立匹配结果，其中 7 项为真实 Agent `PASSED`；同版本单次汇总回归尚未完成；
