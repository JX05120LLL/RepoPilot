...
当前候选版本：`0.1.16 / Pre-Alpha`。第一版 MVP 的后端闭环已经跑通；Windows x64 release 与 NSIS 安装包已成功构建，并完成隔离目录中的安装、启动、sidecar 退出和卸载烟雾测试。桌面端采用 Windows GUI 子系统和无控制台的 Python sidecar，已安装应用启动时不会额外弹出命令行窗口。当前版本支持在同一对话中连续执行多个串行任务，并将终态总结直接展示在聊天流中；当前代码版本的 15 项端到端评测已一次性执行并全部符合预期，升级迁移等交付验收仍在进行，尚不建议用于生产仓库。
