...
def create_app(
    ...
    # 接线片 ③b-1（DR-027）：用 OrchestratedGraphRunner 包住 GraphRunner，
    # 把槽位/租约/心跳/持久化/取消/失败/指标收进正式组件。
    # feature flag 默认关——开时走编排外壳，关时走原 runner（回滚路径）。
    from repopilot_guard.graph_impl.orchestrated_runner import OrchestratedGraphRunner

    agent_handle_mode = os.environ.get("REPOPILOT_AGENT_HANDLE_MODE", "") == "1"
    orchestrated = OrchestratedGraphRunner(
        runner=runner,
        store=store,
        conversations=conversations,  # type: ignore[arg-type]
        slots=app.state.task_slots,
        observability=observability,
    )
    # 接线片 ③b-2：常驻管家注册表（惰性创建 handle，同会话复用）。
    # RequestFactory 在每次 get_or_create 时由请求注入（会话级字段固定）。
    from repopilot_guard.graph_impl.handle_registry import HandleRegistry as _HandleRegistry

    agent_handle_registry: _HandleRegistry | None = None  # 惰性初始化，首次 handle 请求时创建
        ...
        try:
            ...
            def run_in_background() -> None:
                if agent_handle_mode:
                    # 接线片 ③b-1：编排职责已在 OrchestratedGraphRunner 里，
                    # 闭包只起线程调 orchestrated.run（异常由组件内收容）。
                    try:
                        orchestrated.run(request, thread_id, grant)
                    except Exception:
                        # 编排组件已写回失败状态；这里只兜底防止线程异常外泄。
                        pass
                    return
        ...
        try:
            ...
            def resume_in_background() -> None:
                if agent_handle_mode:
                    ...
                    try:
                        orchestrated.resume(thread_id, body.approved, **resume_kwargs)
                    except Exception:
                        pass
                    return
                heartbeat_stop = Event()
                heartbeat = _start_lease_heartbeat(store, thread_id, heartbeat_stop)
                try:
                    resume_kwargs_legacy: dict[str, object] = {
                        "decision": body.resolved_decision(),
                        "comment": body.comment,
                    }
                    # 兼容尚未升级的只读 runner；真实执行审批才传递文件选择。
                    if body.selected_patch_paths is not None:
                        resume_kwargs_legacy["selected_patch_paths"] = body.selected_patch_paths
                    result = runner.resume(thread_id, body.approved, **resume_kwargs_legacy).to_dict()
                    stored = store.sync_graph_result(result, execution_finished=True)
                    if stored.cancellation_requested_at:
                        store.complete_cancellation(thread_id)
                        stored = store.get(thread_id)
