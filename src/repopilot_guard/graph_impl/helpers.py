...
_TOOL_OBSERVATION_MARKER = "受控工具返回的研究证据"
_LEDGER_ASSERT_ENV = "REPOPILOT_LEDGER_ASSERT"


class LedgerAssertionError(RuntimeError):
    """模型可见消息与已记录事件账本不一致（model-visible ⟺ logged 断言失败）。"""

    def __init__(self, observed: int, logged: int) -> None:
        super().__init__(f"LEDGER_MISMATCH observed={observed} logged={logged}")
        self.code = "MODEL_VISIBLE_LOGGED_MISMATCH"
        self.observed = observed
        self.logged = logged


def _tool_observation_count(messages: list[dict[str, str]]) -> int:
    """统计模型可见消息中的工具观测条数：受控工具证据回填或 BLOCKED tool 消息。"""
    ...
    for message in messages:
        ...
        if role == "tool":
            count += 1
        elif role == "user" and str(message.get("content", "")).startswith(_TOOL_OBSERVATION_MARKER):
            count += 1
    return count


def _logged_tool_call_count(tool_events: list[dict[str, object]]) -> int:
    return sum(1 for event in tool_events if event.get("type") == "TOOL_CALL")


def assert_model_visible_logged(state: GraphState) -> None:
    """model-visible ⟺ logged：以 checkpoint ``messages`` 为源、``tool_events``（Evidence）交叉校验。

    模型可见的每一条工具观测都必须能在账本中找到对应的 TOOL_CALL 事件；
    数量不一致说明账实不符（账本丢事件或消息被重放），默认 fail-loud 抛错误。
    ``REPOPILOT_LEDGER_ASSERT=warn`` 降级为告警，``off`` 关闭。
    """

    observed = _tool_observation_count(state.get("messages", []))
    logged = _logged_tool_call_count(state.get("tool_events", []))
    if observed == logged:
        return
    mode = os.environ.get(_LEDGER_ASSERT_ENV, "strict").strip().lower()
    if mode == "off":
        return
    error = LedgerAssertionError(observed, logged)
    if mode == "warn":
        logging.getLogger(__name__).warning("model-visible⟺logged 断言失败：%s", error)
        return
    raise error
