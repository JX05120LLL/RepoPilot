...
桌面端默认提供普通多轮对话。普通聊天只能使用模型文本生成能力，不能读取仓库、调用工具、创建工作区或改变权限。用户须显式切换到“分析代码”或“修改代码”：前者在只读工具研究后直接生成带来源总结，不需要计划审批；后者才进入计划审批和执行审批。任务的终态总结会写入当前聊天流，后续消息可继续围绕同一目标对话，但同一会话同一时刻最多运行一个 Agent 任务。
